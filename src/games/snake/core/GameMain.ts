...
export class SnakeGame implements GameModule {
	...
	public async init(canvasParent: HTMLDivElement): Promise<void> {
		...
		await this.pixiApp.init(canvasParent, width, height, Palette.bg);
	...
	private setupRun(): void {
		...
		const movementSystem = new MovementSystem(world, holder, snakeId, STEP_INTERVAL_MS);
		const bitRegisterSystem = new BitRegisterSystem(world, holder, rng, snakeId);
		const bitTokenSystem = new BitTokenSystem(world, holder, rng, snakeId, MAX_ACTIVE_TOKENS);
